...
function phoneStepHTML(x){
  ...
  const src=x.checked?`<div class="phSrc">Resale value from ${m.kind==="list"?`<b>${esc(srcName(m.src))}</b>, checked ${esc(fmtDay(m.date))}. ${srcLink(m.src)}`:m.kind==="shot"?`${m.n} sold on ${esc(m.site||"the sold page")}. ${srcLink(m.url,"See those sales")}`:m.kind==="retail"?`${money(m.retail)} new retail, taken to ${(m.pct||retailPct())}% for used. Not a sold price.`:m.kind==="found"?`Looked up: used ones go ${money(m.lo)}&ndash;${money(m.hi)}${m.where?` (${esc(m.where)})`:""}. ${m.conf==="h"?"From sold prices.":m.conf==="m"?"From a price guide, not sold prices.":"From asking prices \u2014 check it."}`:m.kind==="seen"?`${m.n} shelf tag${m.n===1?"":"s"} you recorded, asking ${money(m.lo)}&ndash;${money(m.hi)}. Taken a step down, because an ask is not a sale.`:"the price you typed."}</div>`:"";
